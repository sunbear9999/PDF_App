...
class TTSTab(QWidget):
    ...
    def _build_ui(self):
        ...
        self.btn_generate.clicked.connect(self.request_generation)
    ...
    def request_generation(self):
        """Emits an intent to the background service instead of spawning a thread."""
        text_to_read = self.text_editor.toPlainText().strip()
        text_to_read = sanitize_extracted_text(text_to_read, collapse_whitespace=True)
        if not text_to_read:
        ...
        # Fire and forget. The service handles the rest.
        self.bus.tts_action_requested.emit(
            TTSIntent.GENERATE,
            TTSPayload(text=text_to_read, voice_file=voice_file, speed=speed)
        )

    def _handle_status_update(self, event: TTSStatus, payload: TTSStatusPayload):
        """Reacts to state changes from the background service."""
        status = event
        msg = payload.get("msg", "")

        if status == TTSStatus.VOICES_LOADED:
            self.voice_combo.clear()
            self.voice_mapping = payload.get("voices", {})
            for display_name in self.voice_mapping.keys():
        ...
        if status == TTSStatus.RUNNING:
            color = self.theme['warning'] if self.theme else "#ffaa00"
            self.status_lbl.setStyleSheet(f"color: {color}; margin-top: 5px;")
        elif status == TTSStatus.COMPLETE:
            self.btn_generate.setEnabled(True)
            color = self.theme['success'] if self.theme else "#00cc66"
            self.status_lbl.setStyleSheet(f"color: {color}; margin-top: 5px;")
        elif status == TTSStatus.ERROR:
            self.btn_generate.setEnabled(True)
