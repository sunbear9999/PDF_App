...
def _inject_workspace_plugin_items(menu: QMenu, view, context_type: str) -> None:
    ...
    ws_id = getattr(view, "current_workspace_id", 0) or 0
    sel_nodes = [n.node_id for n in view._selected_nodes()] if hasattr(view, "_selected_nodes") else []

    mount_key = f"context_menu:workspace:{context_type}"
    specs = list(action_registry.iter_mount(mount_key))
    if not specs:
        return
    ...
    ctx = ContextMenuContext(
        context_type=f"workspace:{context_type}",
        selected_ids=sel_nodes,
...
def build_selected_nodes_context_menu(view, parent_widget, selected_nodes):
    ...
    _inject_workspace_plugin_items(menu, view, "selection")
...
def build_node_context_menu(view, parent_widget, node, selected_nodes, connect_source):
    ...
    menu.addSeparator()
    menu.addMenu(build_ai_menu(view, menu))
    _inject_workspace_plugin_items(menu, view, "node")
...
def build_edge_context_menu(view, parent_widget, edge):
    ...
    menu.addSeparator()
    menu.addMenu(build_ai_menu(view, menu))
    _inject_workspace_plugin_items(menu, view, "edge")
