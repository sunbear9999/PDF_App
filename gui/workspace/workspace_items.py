...
class Node(QGraphicsRectItem):
    def __init__(
        ...
        # Active controller custom UI (embedded via QGraphicsProxyWidget)
        self.proxy_custom_ui = None
        self._custom_ui_height = 0
        if self.ontology_registry:
            ctrl_meta = self.ontology_registry.get_active_controller(self.entity_type or "")
            if ctrl_meta:
                try:
                    custom_widget = ctrl_meta["controller"].get_custom_ui(dict(self.entity_properties))
                    if custom_widget is not None:
                        ...
                        self._custom_ui_height = hint.height() if hint.isValid() and hint.height() > 0 else 60
    ...
    def refresh_layout(self):
        ...
        custom_ui_h = self._custom_ui_height
        ...
        # Position custom UI widget (always visible when present)
        ...
        if editing_note:
            ...
            self.note_edit_item.setPos(margin, margin + status_h + metric_h + custom_ui_h)
            ...
            if self.is_source_backed_entity() and self.quote_text():
                ...
                self.text_item.setVisible(True)
                self.text_item.setPos(margin, margin + status_h + metric_h + custom_ui_h + editor_height + 8)
            ...
            needed_height = max(self.base_height, editor_height + display_height + (margin * 2) + status_h + metric_h + custom_ui_h + 58)
            ...
            toolbar_width = self.toolbar_widget.sizeHint().width()
            needed_width = max(needed_width, toolbar_width + (margin * 2))
            self.text_item.setPos(margin, margin + status_h + metric_h + custom_ui_h)
            ...
            needed_height = max(self.base_height, doc_height + (margin * 2) + status_h + metric_h + custom_ui_h + 46)
            ...
            max_h = max(10, self.base_height - (margin * 2) - status_h - metric_h - custom_ui_h)
            self.text_item.setPos(margin, margin + status_h + metric_h + custom_ui_h)
