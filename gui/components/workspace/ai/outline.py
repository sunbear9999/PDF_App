...
from gui.components.workspace.dialogs import OutlineDialog
from core.ai_outline_worker import AIOutlineWorker


class WorkspaceAIOutlineHandler:
    ...
    def trigger_generate_outline(self):
        ...
        if not target_nodes:
            QMessageBox.warning(
                v,
                "No Nodes",
                "Please add or select some notes in the workspace first.",
            )
            return
        ...
        v.loading_label.setText(
            "✨ AI is analyzing argument structure and drafting outline...\nThis may take a moment."
        )
        v.loading_overlay.resize(v.viewport().size())
        v.loading_overlay.show()

        if getattr(v, "outline_worker", None) and v.outline_worker.isRunning():
            v.outline_worker.stop()
            v.outline_worker.wait()

        v.outline_worker = AIOutlineWorker(
            llm_manager,
            model,
            nodes_data,
            edges_data,
        ...
        v.outline_worker.finished.connect(
            v._on_generate_outline_finished,
            Qt.ConnectionType.QueuedConnection,
        )
        v.main_window.thread_manager.register_worker("ai_outline_worker", v.outline_worker)
        v.outline_worker.start()
    ...
    def _on_generate_outline_finished(self, outline_text, error_msg):
        ...
        if error_msg:
            QMessageBox.warning(v, "Outline Generation Failed", error_msg)
            return

        dialog = OutlineDialog(outline_text, v)
