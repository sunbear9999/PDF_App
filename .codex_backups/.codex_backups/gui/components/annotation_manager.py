...
class AnnotationManager(QObject):
    ...
    def _on_active_model_changed(self, _event, payload):
        model_name = payload.get("model_name", "") if hasattr(payload, "get") else ""
        if model_name:
   ...
   # gui/components/annotation_manager.py -> AnnotationManager class
    ...
    def _dispatch_ai_blueprint(self, action_type: str, text: str):
        ...
        active_model = self._active_model or (
            main_window._get_active_ai_model() if hasattr(main_window, "_get_active_ai_model") else "gemma4:e2b"
        )
