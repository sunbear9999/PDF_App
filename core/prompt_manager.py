...
class PromptManager:
    ...
    DEFAULT_PROMPTS = {
        ...
        "Graph Analysis Synthesis Query": "Numbered selected quotes:\n{master_input}\n\nWrite a concise synthesis plan for the requested analysis mode. Do not make quote-by-quote mini-arguments. First decide which quotes prove the same point, which quotes are merely examples/data, and which idea should organize the whole graph. Then write one connected structure:\n1. one central conclusion, thesis, organizing relationship, or top-level finding;\n2. 2-4 broad support themes that directly support, explain, qualify, or connect to it;\n3. quote IDs grouped under the support theme they actually prove;\n4. optional tensions/caveats only if they matter to the central structure.\nFor argument-style goals, use as few top-level claims as possible. Treat narrow factual statements, examples, data points, and brief author assertions as evidence unless they summarize several quotes. Do not repeat the same sentence as a claim, reason, and evidence. Keep it compact and evidence-grounded. Do not output JSON.",
        "Graph Analysis Master Query": "Convert the synthesis plan into one coherent connected graph following GRAPH_PLAN and the CONTRACT. Only this final pass should create node types, edge types, confidence/strength fields, and graph structure. Reuse only facts and quote text present in CHUNKS and quote IDs cited in SYNTHESIS. Build one integrated graph, not separate mini-chains. Use one central root item unless SYNTHESIS explicitly requires two. Create only 2-5 broad support/bridge items; each must synthesize several quotes or explain how evidence supports the root. Do not promote every quote or narrow data point into a root/support item. Preserve quote evidence as source_alias leaf nodes when GRAPH_PLAN provides source aliases, and use plenty of those source leaves. Each source leaf should attach to the most relevant support item, and each support item should connect upward to the central root. Avoid claim-to-claim ladders, isolated components, and repeated wording across roles. Aim for 8-18 nodes total, with evidence/source leaves making up about half or more of the nodes when sources are available, and fewer than 26 edges. Return compact JSON only.\n\nSYNTHESIS:\n{argument_synthesis}\n\nCHUNKS:\n{master_input}",
...
""",
        "Graph Analysis Master System": "Build the final graph from the selected quote evidence and the synthesis plan. The chunks contain short numbered quotes and relevance notes, not ontology nodes. You are responsible for creating concise synthesized parent/support/source items, valid edges, and any confidence/strength fields required by the CONTRACT. Deduplicate by meaning and exact quote text. Do not repeat the same wording across different node roles. Do not turn every quote into a parent item; parent/support items should be broader than the evidence beneath them. Output one connected, readable hierarchy unless the synthesis explicitly requires separate components. USER INSTRUCTIONS: {template_instructions}\n\nCONTRACT:\n{template_schema}\n\nDYNAMIC EXAMPLE:\n{schema_few_shot}",
