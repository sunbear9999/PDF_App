...
from core.events.domains.tool_events import (
    TTSIntent, TTSPayload, TTSStatus, TTSStatusPayload,
...
class TTSAppService(QObject):
    ...
    def _handle_intent(self, intent: TTSIntent, payload: TTSPayload):
        ...
        elif intent == TTSIntent.EXTRACT_TEXT:
            self._extract_text(payload)

    def _extract_text(self, payload: TTSPayload):
        path = payload.get("path", "")
        if not path:
            self.bus.tts_text_extracted.emit(
                TTSEvent.TEXT_EXTRACTED,
                TTSEventPayload(error="No PDF path provided."),
            )
            return
        try:
            text = extract_filtered_blocks(
                path,
                payload.get("ignore_headers", True),
                payload.get("start_page", 1),
                payload.get("end_page", 9999),
            )
            text = sanitize_extracted_text(text, collapse_whitespace=True)
            self.bus.tts_text_extracted.emit(
                TTSEvent.TEXT_EXTRACTED,
                TTSEventPayload(text=text, char_count=len(text)),
            )
        except Exception as e:
            self.bus.tts_text_extracted.emit(
                TTSEvent.TEXT_EXTRACTED,
                TTSEventPayload(error=str(e)),
            )
