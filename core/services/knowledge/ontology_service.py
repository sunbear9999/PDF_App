...
class OntologyService(QObject):
    ...
    def handle_entity_intent(self, intent, payload):
        ...
        elif intent == EntityIntent.FETCH_UNVERIFIED:
            self.fetch_unverified_entities(payload)
    ...
    def fetch_unverified_entities(self, payload: EntityPayload):
        """Fetches unverified (AI-generated) entities and emits via discovery_items_changed."""
        entities = self.pm.get_unverified_entities(
            limit=payload.limit,
            entity_type=payload.entity_type,
            offset=payload.offset,
        ) if self.pm else []
        self.bus.discovery_items_changed.emit(
            EntityIntent.FETCH_UNVERIFIED,
            EntityPayload(data={"entities": entities or [], "has_more": False}),
        )
